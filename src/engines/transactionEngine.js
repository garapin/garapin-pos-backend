...
class TransactionEngine {
  ...
  async processTransactions(response) {
    for (const transactionData of response.data.data) {
      ...
      if (
        this.isValidReferenceId(transaction.reference_id) &&
        ...
        try {
          ...
          Logger.errorLog(
            "Gagal menghubungkan ke database atau memproses transaksi",
            error
          );
        }
      }
  ...
  async checkAndSplitTransaction(route, transaction) {
    const transactionDestination = await this.fetchTransactionDestination(
      route,
      transaction
    );
  ...
  async splitTransaction(route, transaction) {
    ...
    try {
      ...
      if (postTransfer.status === 200) {
        Logger.log(
          `Transaction ${transaction.reference_id} successfully split`
        );
      } else {
