...
const createSplitRule = async (req, res) => {
  try {
    ...
    const totalPercentAmount = routes.reduce(
    ...
    if (totalPercentAmount !== 100) {
      return apiResponse(
        res,
        400,
        "total persen bagi-bagi pendapatan harus 100%"
      );
    }
    const totalPercentFeePos = routes.reduce(
    ...
    const TemplateModel = dbTemplate.model(
      "Split_Payment_Rule",
      splitPaymentRuleIdScheme
    );
    ...
    const routesValidate = routes.map((route) => {
      return {
        percent_amount:
          route.percent_amount - (garapinCost / 100) * route.fee_pos,
    ...
    const referenceIdTRX = routes
      .filter((item) => item.type === "TRX")
    ...
    if (response.status === 200) {
      for (const route of response.data.routes) {
        ...
        const splitRuleId = await SplitPaymentRuleIdStore.findOne({
          id_template: id_template,
        });
        if (!splitRuleId) {
...
const addFeeCustomer = async (req, res) => {
  try {
    ...
    const totalPercentFeePos = existingTemplate.routes.reduce(
      (acc, route) => acc + route.fee_pos,
      0
    );
    let updatedTemplate;
...
const deleteTargetTemplate = async (req, res) => {
  try {
    ...
    template.routes = template.routes.filter(
      (data) => data.reference_id !== reference_id
    );
    await template.save();
...
const updateTemplate = async (req, res) => {
  try {
    ...
    const existingRoute = existingTemplate.routes.find(
      (r) => r.reference_id === reference_id
    );
      ...
      if (route.type === "TRX") {
        if (existingTemplate.db_trx === route.reference_id) {
          ...
          const foundTemplate = template.find(
            (temp) => temp.db_trx && temp.db_trx.includes(route.reference_id)
          );
          if (!foundTemplate) {
            const updatedTemplate = await Template.findOneAndUpdate(
              { _id: id, "routes.type": "TRX", status_template: "INACTIVE" },
...
const activationTemplate = async (req, res) => {
  try {
    ...
    const existingTemplate = await Template.findById(id_template);
    const totalPercentAmount = existingTemplate.routes.reduce(
      (acc, route) => acc + route.percent_amount,
      0
    );
    if (totalPercentAmount !== 100) {
