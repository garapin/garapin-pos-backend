...
const createMerchant = async (req, res) => {
  try {
    ...
    let { store_name, email, merchant_role, sender_email } = req.body;

    const mystoredb = await connectTargetDatabase(targetDatabase);
    const myStoreModel = mystoredb.model("Store", storeSchema);
    const mystoreData = await myStoreModel.findOne();
    ...
    if (store_name.length > 20) {
      return apiResponse(
        res,
        400,
        "Nama database tidak boleh lebih dari 30 karakter"
      );
    }

    const existDb = await DatabaseModel.findOne({
      db_parent: targetDatabase,
      email_owner: email,
    });
    if (existDb) {
      ...
      return apiResponse(
        res,
        400,
        `Email sudah terdaftar sebagai ${storeData.merchant_role}`
      );
    }
    const uniqueId = uuidv4().slice(0, 12);
    const storeDatabaseName = `mr-${store_name.replace(/\s+/g, "_")}_${uniqueId}`;
    const dbGarapin = await DatabaseModel({
      db_name: storeDatabaseName,
      email_owner: email,
      db_parent: targetDatabase,
    });
    ...
    const DatabaseMerchant = merchantDatabase.model(
      "Database_Merchant",
      databaseMerchantSchema
    );
    ...
    sendMail(
      ...
      mystoreData.pic_name,
      mystoreData.phone_number,
      sender_email,
      storeDatabaseName
    );
...
const approvalRequestMerchant = async (req, res) => {
  try {
    ...
    if (approve === "Y") {
      const updateStore = await StoreModelInStoreDatabase.updateOne(
        {},
        {
          $set: {
...
const getAllMerchant = async (req, res) => {
  try {
    ...
    const MerchantModel = storeDatabase.model(
      "Database_Merchant",
      databaseMerchantSchema
    );
...
export default {
  createMerchant,
  approvalRequestMerchant,
  getAllMerchant,
  acceptInvitationMerchant,
};
